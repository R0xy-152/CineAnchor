...
### [2026-05-10 AnimateDiff 首次完整跑通 — 小win]

**GPU VAE 回退后，AnimateDiff 正常完成！无降级，无报错。**

| 帧 | ratio | center-edge | 判定 |
|---|---|---|---|
| 0 | 1.20 | -3 | BLUR |
| 1-7 | 1.13~1.28 | +1~+20 | BLUR |

**0/8 CLEAR, mean=1.21 std=0.049**

**分析：**
- 时序一致性极好 (std=0.049)，所有帧质量高度均一
...
**1.7 是 sweet spot** — ratio 1.21 + std 0.049。已回锁 1.7。

---

### [2026-05-10 Phase 1 收官 — mac]

**AnimateDiff 调参结论：**

| scale | ratio | std | 判定 |
|-------|-------|-----|------|
| 1.5 | 1.20 | 0.040 | 时序最佳，信号略弱 |
| **1.7** | **1.21** | **0.049** | **最优平衡点** |
| 2.0 | 1.26 | 0.082 | 时序退化 |

**Phase 1 最终配置锁定：纹理立方体 + AnimateDiff + scale=1.7 + sequential offload**

## Phase 2 方向

scale 参数已挖到底，ratio 瓶颈在**场景几何复杂度**——简单立方体的深度信息不足以让 ControlNet 在 AnimateDiff 平滑下产出高对比度结构。

Phase 2 从两个方向突破：
1. **更丰富的 3DGS 场景**（多物体、凹凸纹理、复杂几何）→ 深度图有更多 ControlNet 可锁定的特征
2. **前端取景器接入 API**（录制真实相机轨迹 → 喂入管线）
...
**小win ✅ 复杂场景已推送** (`scene_complex.ply`) — 5 个纹理立方体，分散 [-2,2]³

## 🔴 验证任务 — 小win

```bash
git pull
python generate_cube_splat.py          # 生成复杂场景 PLY
python e2e_test.py                      # AnimateDiff + 复杂场景
```

**关键验证：** AnimateDiff + 复杂场景的 ratio 能否突破简单纹理立方体的 1.21 天花板。
...
### [2026-05-20 环境重建 + 管线验证 — 小win]

**任务：** 重新配置 CUDA 环境并验证渲染管线完好。

**环境配置过程（CUDA 环境之前被清理了）：**
1. `git pull` — 成功拉取 17 个文件，包含 app/、static/、PRD 等新模块
2. PyTorch: CPU → CUDA (2.6.0+cu124)
3. CUDA Toolkit: 系统有 13.2（不匹配 cu124），conda 安装 12.4.1
4. gsplat JIT 编译修复：
   - 添加 MSVC bin/include/lib 路径（D:\VisualStudio + Windows Kits）
   - 修复 `-Wno-attributes`（GCC 标志，MSVC 不认 → 换 `/wd` 标志）
   - 添加 conda CUDA lib 路径（`/lib` vs `/lib/x64` 差异）
   - 添加 `-ccbin` 指向 MSVC 编译器
5. 所有依赖已安装（diffusers, transformers, accelerate, controlnet_aux, opencv-python, plyfile）

**管线验证结果：**

| 步骤 | 结果 | 详情 |
|------|------|------|
| PLY 生成 | ✅ | 5 个 PLY (纹理立方体等) |
| 深度图渲染 | ✅ | 8 帧, 全局归一化, GPU RTX 4060 |
| ControlNet RGB | ✅ | 8 帧逐帧生成, ~6.4 it/s, scale=1.7 |
| RAFT 插值 | ✅ | 8→22 帧 (3x) |
| 视频合成 | ✅ | `videos/e2e_test_output.mp4` (15KB, 8fps) |

**深度图质量：** 全 8 帧 raw depth [0.900, 1.000], 全局归一化 [0.936, 1.000] — 区间极窄，Frame 7 接近饱和（unique=1）。

**注意：**
- AnimateDiff motion adapter 下载失败（SSL EOF → fallback 逐帧），HuggingFace 连接不稳定
- 用户检查后发现 Meshy AI 免费额度不支持 API 调用
- 需要稳定的代理/VPN 来访问 HuggingFace 和外部 API
...
$env:LIB = "$msvc\lib\x64;C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\ucrt\x64;..."
$env:PATH = "$msvc\bin\Hostx64\x64;$env:CUDA_HOME\bin;$env:PATH"
python e2e_test.py --sd15
```

**注意：** gsplat 的 `_backend.py` (site-packages) 有 3 处修改：`-ccbin` 路径、`/wd` 替代 `-Wno-attributes`、conda lib 路径。如果 pip 重新安装 gsplat 需重新应用这些修改。
...
| 指标 | SD 1.5 AnimateDiff | SDXL per-frame |
|------|-------------------|----------------|
| ratio | **1.21** | 0.53 ❌ |
| std | **0.049** | 0.115 ❌ |
| 推理 | ~5s/frame | ~8.6s/frame @768 |
| VRAM | OK | OK (cpu_offload) |
